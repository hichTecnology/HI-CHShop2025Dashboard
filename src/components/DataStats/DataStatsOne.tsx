...
import { User } from "@/app/api/modal";
...
const DataStatsOne: React.FC<dataStats> = () => {
  ...
  const [data, setData] = useState<SalesTotal | null>(null);
  ...
  useEffect(() =>{
    const fetchSaldes = async () => {
      fetch('/api/sales-total')
      .then((res) => res.json())
      .then((json: SalesTotal) => {
        setData(json);
    ...
    const fetchViews = async () => {
      const res = await fetch(`${apiUrl}/analytics/visitors`, {})
      const views = await res.json()
      setViews(views.visitors)
