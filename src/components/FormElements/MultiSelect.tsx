...
interface DropdownProps {
  id: string;
  colors: string[]
  sendColorToParent: (data: Color) => void;
  removeColor: (option: Color) => void
}

const MultiSelect: React.FC<DropdownProps> = ({ id,colors ,sendColorToParent,removeColor}) => {
    ...
    const loadOptions = () => {
      ...
      if (select ) {
        ...
        for (let i = 0; i < select.options.length; i++) {
          newOptions.push({
            name: select.options[i].value,
            cod: select.options[i].innerText,
  ...
  const select = (index: number, event: React.MouseEvent) => {
    ...
    sendColorToParent(newOptions[index])
  };

  const remove = (index: number,option: Color) => {
    const newOptions = [...options];
    const selectedIndex = selected.indexOf(index);
     removeColor(option)
  ...
  return (
    ...
    <div className="relative z-20">
      ...
      <div>
        ...
        {colors.length && colors.map((index) => (
