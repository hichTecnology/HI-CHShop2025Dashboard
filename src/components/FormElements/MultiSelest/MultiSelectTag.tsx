...
interface DropdownProps {
  id: string;
  tags: Tag[]
  sendTagToParent: (data: Tag) => void;
  removeTag: (option: Tag) => void
}

const MultiSelectTag: React.FC<DropdownProps> = ({ id,tags ,sendTagToParent,removeTag}) => {
      ...
      if (select ) {
        ...
        for (let i = 0; i < select.options.length; i++) {
          newOptions.push({
            ...
            selected2: select.options[i].hasAttribute("selected"),
          });
        }
  ...
  const select = (index: number, event: React.MouseEvent) => {
    ...
    if (!newOptions[index].selected2) {
      newOptions[index].selected2 = true;
      newOptions[index].element2 = event.currentTarget as HTMLElement;
      setSelected([...selected, index]);
      
    } else {
      const selectedIndex = selected.indexOf(index);
      if (selectedIndex !== -1) {
        newOptions[index].selected2 = false;
    ...
    setOptions(newOptions);
    prova.push(newOptions[index])
    
    sendTagToParent(newOptions[index])
  };

  const remove = (index: number,option: Tag) => {
    const newOptions = [...options];
    const selectedIndex = selected.indexOf(index);
     removeTag(option)
                        ...
                        >
                          <div
                            className={`relative flex w-full items-center border-l-2 border-transparent p-2 pl-2 ${
                              option.selected2 ? "border-primary" : ""
                            }`}
                          >
