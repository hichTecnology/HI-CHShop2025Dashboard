...
interface DropdownSizeProps {
  ...
  removeSize: (option: Size) => void
}

const MultiSelectSize: React.FC<DropdownSizeProps> = ({ id,sizes ,sendSizeToParent,removeSize}) => {
  ...
  useEffect(() => {
    const loadOptions = () => {
      const select1 = document.getElementById(id) as HTMLSelectElement | null;
      if (select1 ) {
        const newOptions: Size[] = [];
        for (let i = 0; i < select1.options.length; i++) {
          newOptions.push({
            name: select1.options[i].innerText,
            id : select1.options[i].value,
            selected1: select1.options[i].hasAttribute("selected"),
          });
        }
  ...
  const select = (index: number, event: React.MouseEvent) => {
    ...
    if (!newOptions[index].selected1) {
      newOptions[index].selected1 = true;
      newOptions[index].element1 = event.currentTarget as HTMLElement;
      setSelected([...selected, index]);
      
    } else {
      const selectedIndex = selected.indexOf(index);
      if (selectedIndex !== -1) {
        newOptions[index].selected1 = false;
    ...
    sendSizeToParent(newOptions[index])
  };

  const remove = (index: number,option: Size) => {
    const newOptions = [...options];
    const selectedIndex = selected.indexOf(index);
     removeSize(option)
  ...
  const selectedValues = () => {
    return selected.map((option) => options[option].name);
  };
                    ...
                    {selected.map((index) => (
                      ...
                      >
                        ...
                        <div className="flex flex-auto flex-row-reverse">
                          <div
                            onClick={() => remove(index,options[index])}
                        ...
                        >
                          <div
                            className={`relative flex w-full items-center border-l-2 border-transparent p-2 pl-2 ${
                              option.selected1 ? "border-primary" : ""
                            }`}
                          >
