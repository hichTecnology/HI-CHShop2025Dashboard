...
interface updateVarienteProps {
  id : string;
  link : string
  sendVarienteToParent: (data: boolean) => void;
  
}

const UpdateVariente  :React.FC<updateVarienteProps> = ({ id,link,sendVarienteToParent}) => {
  const route = useRouter()
  const [variente,setVariente] = useState<Color>()
    ...
    const schemaCategory = z.object({
      ...
      useEffect(() =>{
        const fetchProducts = async () => {
          ...
          try {
            const res = await fetch(`${apiUrl}/varientes/${id}`); // Sostituisci con la tua API
            if (!res.ok) {
      ...
      const onSubmit :SubmitHandler<IssinUp> = async (category) =>{
          ...
          try {
            ...
            const response = await fetch(`${apiUrl}/varientes/${id}`, {
              method: 'PATCH',
  ...
  return (
    <div  className="fixed inset-0 z-999 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
      <div className="p-8 rounded-[10px] border border-stroke bg-white shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        <button onClick={()=>sendVarienteToParent(false)} className=' p-2 m-2 bg-[#4b313d]  rounded-md'>
          <VscClose />
        </button>
